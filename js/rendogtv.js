...
setTimeout(() => {
  ...
  const handleMessage = (element) => {
    ...
    const exitingModButton = messageLine.querySelector("button.mod-icon");

    if (!exitingModButton) {
      return;
    }

    const modIconsContainer =
      exitingModButton.parentElement?.parentElement?.parentElement;

    const punishableDiv = document.createElement("div");
    const punishableButton = document.createElement("button");
    punishableButton.title = "Punish user";
    ...
    modIconsContainer.prepend(punishableDiv);
  };
